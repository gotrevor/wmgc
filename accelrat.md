...
So why did it stay a curiosity?  Partly plumbing, though less than you would think.  The AGA's own tournament resources page says flatly that *"Accelerate no longer works with AGA ratings software, and is no longer actively supported"* - and then, a few paragraphs later in its end-of-tournament checklist, tells directors *"if you use Accelrat or MGA GoTD you can generate a ratings report automatically."*  Both sentences are on the same page.

The second one is closer to the truth, because the barrier here is small by construction.  The AGA's ratings submission format is, in its own words, *"so simple that, for a small number of games, you could just type the data as part of your e-mail message."*  You send a `TOURNEY` line with name, dates and rules, a `PLAYERS` block of AGA ID / name / rank, and a `GAMES` block of white, black, winner, handicap stones, komi.  That is nearly the shape AccelRat already writes.  It is not a coincidence: Matthews spent twenty years as the AGA's ratings statistician, and the worked example in the AGA's own submission documentation uses "Matthews, Paul" as player 489.  Whatever has drifted apart is a field mapping, not a wall.

The larger reason is institutional.  Swiss-McMahon has published AGA **pairing protocol standards** and a documented interface for programmers, and the AGA offers to certify programs against them.  The accelerated system has no such document, so every director who wanted to run one was on his own, and every programmer building a pairing tool had a spec for one system and not the other.  Meanwhile there is no shortage of pairing software - the AGA has at various times pointed directors at [OpenGotha](https://www.opengotha.info/), MGA GoTD, Go Clubs Online, WinTD, McMahon 3.3 and Agatha, and notes that it certifies none of them as "AGA compliant."  Ranks won the tooling, and the tooling is what a tired TD reaches for at 9am on a Saturday.
